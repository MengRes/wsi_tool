...
from wsi_core.wsi_utils import screen_coords, isBlackPatch, isWhitePatch, to_percentiles, initialize_hdf5_coords_only, savePatchIter_coords_only
...
class WholeSlideImage(object):
    def __init__(self, path):
        # self.name: wsi name without suffix
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, 
                      close = 0, use_otsu=False, filter_params={'a_t':100}, 
    ...
    def createPatches_bag_hdf5(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        """
        Extract patches and save coordinates to HDF5 file
        """
        # Check if tissue segmentation has been performed
        if not hasattr(self, 'contours_tissue') or self.contours_tissue is None:
            raise ValueError("Tissue segmentation must be performed before patch extraction. Call segmentTissue() first.")
        
        # Setup logging
        log_file = os.path.join(save_path, f"{self.name}_patch_extraction.log")
        logging.basicConfig(
            level=logging.INFO,
        ...
        logging.info(f"Starting patch extraction for: {self.name}")
        logging.info(f"Parameters: patch_level={patch_level}, patch_size={patch_size}, step_size={step_size}")
        ...
        # Use coordinates-only mode
        logging.info("Using coordinates-only mode")
        return self._createPatches_coords_only(save_path, patch_level, patch_size, step_size, **kwargs)

    def _createPatches_coords_only(self, save_path, patch_level, patch_size, step_size, **kwargs):
        """Extract patches and save only coordinates"""
        contours = self.contours_tissue
        contour_holes = self.holes_tissue
        
        # Type check to ensure contours is not None and is a list
        if contours is None:
            raise ValueError("contours_tissue is None. Please call segmentTissue() first.")
        
        logging.info("Starting contour processing...")
        elapsed = time.time()
        total_patches = 0
        
        for idx, cont in enumerate(contours):
            logging.info(f"Processing contour {idx+1}/{len(contours)}")
            patch_gen = self._getPatchGenerator(cont, idx, patch_level, save_path, patch_size, step_size, coords_only=True, **kwargs)
            
            if self.hdf5_file is None:
                try:
                    first_patch = next(patch_gen)
                    logging.info(f"First patch coordinates: ({first_patch['x']}, {first_patch['y']})")
                    
                except StopIteration:
                    logging.warning(f"Contour {idx} is empty, skipping")
                    continue

                file_path = initialize_hdf5_coords_only(first_patch, save_coord=True)
                self.hdf5_file = file_path
                logging.info(f"Created HDF5 file: {file_path}")

            patch_count = 0
            for patch in patch_gen:
                savePatchIter_coords_only(patch)
                patch_count += 1
            
            total_patches += patch_count
            logging.info(f"Contour {idx} extracted {patch_count} patches")

        elapsed_time = time.time() - elapsed
        logging.info(f"Extraction completed, total {total_patches} patches, time elapsed {elapsed_time:.2f} seconds")
        logging.info(f"HDF5 file saved at: {self.hdf5_file}")
        
        return self.hdf5_file

    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, patch_size = 256, step_size = 256, 
                           custom_downsample = 1, white_black = True, white_thresh = 15, black_thresh = 50, 
                           contour_fn = 'four_pt', use_padding=True, coords_only=False):
        ...
        for y in range(start_y, stop_y, step_size_y):
            for x in range(start_x, stop_x, step_size_x):
                ...
                # If only coordinates are needed, skip image reading and quality checks
                if coords_only:
                    patch_info = {'x':x // (patch_downsample[0] * custom_downsample), 
                                  ...
                                  'name':self.name, 
                                  'save_path':save_path,
                                  'patch_PIL':None}  # Add patch_PIL key, set to None
                else:
    ...
    def process_contours(self, save_path, patch_level = 0, patch_size = 256, step_size = 256, **kwargs):
        ...
        # Type check to ensure contours_tissue is not None
    ...
    def get_seg_mask(self, region_size, scale, use_holes=False, offset=(0,0)):
        ...
        for idx in range(len(contours_tissue)):
            cv2.drawContours(image=tissue_mask, contours=contours_tissue, contourIdx=idx, color=(1,1,1), offset=offset, thickness=-1)

            if use_holes:
                cv2.drawContours(image=tissue_mask, contours=contours_holes[idx], contourIdx=-1, color=(0,0,0), offset=offset, thickness=-1)
