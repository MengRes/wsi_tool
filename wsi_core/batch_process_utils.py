...
def initialize_df(slides, seg_params, filter_params, vis_params, patch_params, 
	...
	if use_heatmap_args:
		...
		default_df_dict.update({'x1': np.empty((total)).fill(np.NaN),  # pyright: ignore[reportAttributeAccessIssue]
			'x2': np.empty((total)).fill(np.NaN),  # pyright: ignore[reportAttributeAccessIssue]
			'y1': np.empty((total)).fill(np.NaN),  # pyright: ignore[reportAttributeAccessIssue]
			'y2': np.empty((total)).fill(np.NaN)}) # pyright: ignore[reportAttributeAccessIssue]
